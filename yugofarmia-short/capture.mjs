...
const parseCoverage = (text, href) => {
  ...
  return {
    ...
    observations,
    source,
    periodStart: dateLines[0] ?? '',
    periodEnd: dateLines[1] ?? dateLines[0] ?? '',
    latestMedian,
    href,
    rawText: text,
  };
...
try {
  ...
  for (const product of products) {
    ...
    const productImage = images.find((image) => /r2\.dev\/products\/products\//i.test(image.src))
      ?? images.find((image) => image.alt.toLowerCase().includes(product.label.toLowerCase().split(' ')[0]));
    ...
    const coverageLinks = await page.locator('a[href*="/en/prices?product="]').evaluateAll((elements) => elements.map((element) => ({
    ...
    for (const item of coverage.slice(0, 3)) {
      ...
      try {
        ...
        resultPages.push({
          ...
          source: item.source,
          href: item.href,
          prices: extractPriceStrings(resultText),
          bodyText: resultText,
        });
      } catch (error) {
